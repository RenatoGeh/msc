...
tractably computing the \map{}. \Cref{fig:determinism} shows an example of a deterministic Gaussian
mixture model.
...
\begin{figure}[t]
  \begin{subfigure}[t]{0.39\textwidth}
    ...
    \begin{tikzpicture}
      ...
      \begin{axis}[
        no markers, domain=-1:6, samples=100,
  ...
  \begin{subfigure}[t]{0.29\textwidth}
    ...
    \begin{tikzpicture}
      \newSumNode[fill=boxred!70]{s}{0,0};
      \newGaussNode[fill=boxteal]{g1}{$(s) + (-1.5,-1.0)$};
      \newGaussNode[fill=boxorange!80]{g2}{$(s) + (0,-1.0)$};
      \newGaussNode[fill=boxpurple!60]{g3}{$(s) + (1.5,-1.0)$};
      \draw[edge] (s) edge (g1);
      \draw[edge] (s) edge (g2);
      \draw[edge] (s) edge (g3);
      \node at ($(s) + (-1.2,-0.35)$) {\scriptsize$.40$};
  ...
  \begin{subfigure}[t]{0.25\textwidth}
    ...
    \caption{}
  \end{subfigure}
  \caption{A Gaussian mixture model is deterministic if the components do not overlap. Graph
    \uncaption{(a)} shows a deterministic Gaussian mixture model represented as a PC in
    \uncaption{(b)} and whose components $\psi_1$, $\psi_2$ and $\psi_3$ are Gaussians truncated in
    a way that they do not overlap. Given any valuation, at most one of the components (sum node's
    children) must have a positive value \uncaption{(c)}.}
...
Diagrams (SDDs, \cite{darwiche11}). \Cref{fig:logic} shows two SDDs encoding the same knowledge
