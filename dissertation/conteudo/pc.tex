...
entire scope of a node, it is said to be a \emph{complete assignment}, or a \emph{partial
assignment} otherwise. When we say that an input node must be \emph{tractable}, we mean that it
must \emph{at worst} be able to compute the \emph{probability evidence} and \emph{marginal
probability} of any assignment $\set{x}$ in polynomial time. We define probability of evidence as a
complete assignment $\set{x}$'s likelihood according to the probabilistic circuit, while the
marginal probability is the marginalization of a partial assignment's remaining random variables
$\set{Y}$: $\int_{\set{y}}p(\set{x},\set{y})\dif\set{y}$. The value, be it the probability of
evidence or marginal probability, of an input node is the same of the distribution it represents
under a given evidence. We abuse notation and say that $\Leaf_p(\set{x})=\int_{\set{y}}p(\set{x},
\set{y})\dif\set{y}$ is the marginal of partial assignment $\set{x}$. \Cref{eg:gaussians} shows an
instance of a Gaussian distribution as a PC.
...
probability distribution given by a node $\Node$'s induced distribution.
...
bottom-up approach. The notion of scope is essential to many properties in PCs, the first of which
...
Essentially, a smooth probabilistic circuit guarantees that every child in a sum node $\Sum$ has
the same scope as their siblings and consequently their parent $\Sum$ as well. \Cref{eg:mixgauss}
shows a case of a Gaussian mixture model as a smooth PC.
...
$\Prod(\set{x})=\prod_{\Child\in\Ch(\Prod)} \Child(\set{x})$. We now introduce
\emph{decomposability}, a structural constraint on products.
...
smooth and decomposable, with these two constraints always assumed to hold.
...
informally touched on: probability of evidence and marginal probability. Given a probabilistic
circuit $\mathcal{C}$ whose root is $\Node$, we shall use $\mathcal{C}$ and its root $\Node$
interchangeably when there is no need for a distinction. We refer to the probability of evidence by
the shorthand \evi{}, computable by a bottom-up evaluation of each node as shown in \Cref{alg:evi}.
Likewise, marginals are denoted by \mar{}, and are computed bottom-up just like in \evi{} with the
exception of inputs, where we marginalize over the unseen variables, as shown in \Cref{alg:mar}. In
both cases, the computation is done in linear time on the number of edges of the circuit.

The conditional probability is another query 
...
Before we address
querying in PCs, we must first discuss some important concepts that often come up in PC literature.
Mainly, we are interested in defining two notions here: induced subcircuits and variable trees.

probabilistic circuit $\mathcal{C}$ contains no consecutive sums or products (i.e. for every sum
all of its children are either inputs or products and respectively for products) then it is said to
be a \emph{standard} form circuit. Any PC can be transformed into a \emph{standardized} circuit, a
process we call \emph{standardization} (see \Cref{thm:standard}).
...
\begin{figure}[t]
  ...
  \end{subfigure}\begin{subfigure}[t]{0.245\textwidth}
    ...
    \end{center}
  \end{subfigure}
  \caption{A probabilistic circuit \uncaption{(a)} and 3 of the 12 possible induced subcircuits
    \uncaption{(b)}.}
  \label{fig:induced}
\end{figure}
...
\begin{restatable}[\cite{poon11,pclec,vergari21}]{theorem}{linevi}
  \label{thm:linevi}
  Let $\mathcal{C}$ be a \emph{smooth} and \emph{decomposable} PC. Any one of \evi{}, \mar{} or \con{}
