...
\begin{example}[breakable,sidebyside,lefthand width=0.55\textwidth]{Naïve Bayes as probabilistic circuits}{nbayes}
  ...
  \begin{tikzpicture}
    ...
    \newLeafNode[fill=boxteal,label={[label distance=-0.1cm]below right:{\scriptsize$\ind_{y=1}$}}]{i1}{$(p1) + (0.5,-1.5)$};
    \newLeafNode[fill=boxteal,label={[label distance=-0.1cm]below right:{\scriptsize$\ind_{y=2}$}}]{i2}{$(p2) + (0.5,-1.5)$};
    \newLeafNode[fill=boxteal,label={[label distance=-0.1cm]below right:{\scriptsize$\ind_{y=3}$}}]{i3}{$(p3) + (0.5,-1.5)$};
...
Although base queries cover most of the needs of the usual data scientist, for more complex tasks
...
\begin{example}[sidebyside,lefthand width=0.55\textwidth]{Hidden Markov models as probabilistic circuits}{hmm}
  Say we wish to model a temporal structured dependence between three variables, for example a
  subject $X_1$, verb $X_2$ and object $X_3$ phrase in a natural language. Each observation $Y_i$
  is a fragment of a complete sentence composed of all fragments $\set{y}=(y_1,y_2,y_3)$. We can
  do this through the first-order Hidden Markov Model (HMM) on the right, i.e. computing
  \begin{equation*}
