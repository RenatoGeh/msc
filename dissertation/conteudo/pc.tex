...
\begin{example}[sidebyside,lefthand width=0.55\textwidth]{Density estimation trees as probabilistic circuits}{det}
  ...
  \begin{tikzpicture}
    ...
    \draw[edge] (a) -- node[left,pos=0.5] {\scriptsize$1$} (al); \draw[edge] (al) -- (ai);
    \draw[edge] (a) -- node[right,pos=0.5] {\scriptsize$1$} (ar); \draw[edge] (ar) -- (nai);
    ...
    \draw[edge] (al) -- (b);
    \draw[edge] (b) -- node[right,pos=0.5] {\scriptsize$1$} (bl); \draw[edge] (bl) -- (bi);
    \draw[edge] (b) -- node[above right,pos=0.5] {\scriptsize$1$} (br); \draw[edge] (br) -- (nbi);
    ...
    \draw[edge] (br) -- (c);
    \draw[edge] (c) -- node[above left,pos=0.5] {\scriptsize$1$} (cl); \draw[edge] (cl) -- (ci);
    \draw[edge] (c) -- node[left,pos=0.5] {\scriptsize$1$} (cr); \draw[edge] (cr) -- (nci);
...
\begin{figure}[t]
  \begin{subfigure}[t]{0.32\textwidth}
    ...
    \begin{tikzpicture}
      ...
      \node (out) at ($(r) + (0,0.9)$) {\scriptsize\colorbox{boxgreen}{\color{white}$.09$}};
  ...
  \caption{The computation, on a smooth, decomposable and deterministic probabilistic circuit, of
    \evi{} $p(a=1.2,b=3.6,x=1,y=-1)\approx 0.09$ \uncaption{(a)}, \map{}
...
$\mathcal{C}$ by backtracking the graph to the most probable assignments. This maximum induced tree
can be retrieved by a top-down pass selecting the most probable sum child nodes according to the
probabilities set by \map{}. Since $\mathcal{C}$ is decomposable, there cannot exist a node in
$\mathcal{M}$ with more than one parent, meaning it is by construction a tree whose leaves are
input nodes with scopes whose union is the scope of $\mathcal{C}$. This reduces the problem to a
divide-and-conquer approach where each input node is individually maximized.
...
\Cref{eg:hmm} shows a Hidden Markov model as a smooth, deterministic and structure decomposable PC
...
described as smooth, deterministic and structure decomposable PCs.
...
The notion of structure decomposability (or compatibility for that matter) is key to more complex
...
smooth, structure decomposable and deterministic logic circuit as a BDD. Although no structural
