...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    ...
    \State Sort $\set{a}=\set{w}\cdot\set{x}$ for $\set{x}\in\set{D}$ s.t.\ $a_1\leq a_2\leq\dotsb\leq a_n$
    \For{each $i\in\left[n-1\right]$}
      \State $\mu_1 = \frac{1}{i}\sum_{j=1}^i a_i$, \; $\mu_2 = \frac{1}{n-i}\sum_{j=i+1}^n a_i$
      \State $c_i = \sum_{j=1}^i (a_j - \mu_1)^2 + \sum_{j+1}^n (a_j - \mu_2)^2$
    \EndFor
    \State Find $i$ that minimizes $c_i$ and set $\theta = (a_i + a_{i+1})/2$
    \State $\set{S}_1 \gets \{\set{x}\,|\,\forall\set{x}\in\set{D}\wedge\set{w}\cdot\set{x}\leq\theta\}$
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    ...
    \State $\set{S}_1\gets\{\set{x}\,|\,\forall\set{x}\in\set{D}\wedge\set{w}\cdot\set{x}\leq\operatorname{median}(\{\set{z}\cdot\set{w}\,|\,\set{z}\in\set{D}\})+\delta\}$
    \State \textbf{return} $(\set{S}_1,\set{D}\setminus\set{S}_1)$
  \end{algorithmic}
\end{algorithm}
...
threshold projection value that roughly divides dataset $\set{D}$ into two approximately equally
sized subsets. \Cref{alg:splitsid} attempts at finding such a threshold by minimizing the average
...
Albeit our contributions are preliminary, we found that our approach is extremely fast and reaches
...
(akin to decisiont trees) to the model. From a more practical point of view, we point to the work
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    ...
    \NIElse
      ...
      \State Construct products $\Prod_1\gets\Child^{(1)}_1\cdot\Child^{(1)}_2$ and $\Prod_2\gets\Child^{(2)}_1\cdot\Child^{(2)}_2$
      \State \textbf{return} sum $\frac{\set{S}_1}{\set{D}}\cdot\Prod_1+\frac{\set{S}_2}{\set{D}}\cdot\Prod_2$
    \EndNIElse
  \end{algorithmic}
\end{algorithm}

Determinism can be enforced if, for every input node (line \ref{alg:learnrp:line:input} in
\Cref{alg:learnrp}) its support is truncated to the cell induced by all random projections above
it. For the discrete case, we might attribute zero mass to assignments outside its cell,
normalizing the distribution with the remaining mass. However, this is not so trivial in the
general case (i.e.\ in the continuous) as the projections are oblique and the resulting truncated
distributions must have tractable marginalization.

When the domain is discrete, the vtree can be learned by pairwise mutual information as discussed
in \Cref{subsection:learnpsdd}; when in the continuous, we can replace mutual information for
Pearson's correlation or any other similar metric.
