...
recursively call the same procedure on their formulae. \Cref{alg:samplepsdd} shows this recursive
procedure formally. The sampling process of generating primes and their subs is shown in
\Cref{alg:samplepartial}, and goes as follows. To produce primes (and subs), we must look at the
...
An example of such a tree is shown as the right tree in \Cref{fig:partial-b}. We efficiently
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    ...
    \If{$|\Sc(v)|=1$}
      ...
      \IElse{\textbf{return} a Bernoulli distribution input node from $\Sc(v)$}
    \ElsIf{$\phi\equiv\top$}
