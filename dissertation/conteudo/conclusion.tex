...
(\Cref{ch:scalable}). Each tackles the problem of learning PCs from distinct point of views: we
first propose \textproc{SamplePSDD} to learn a circuit from certain knowledge, constructing a
...
competitive performance against the state-of-the-art (\Cref{sec:logical}). Next, we describe a very
...
competitive performance on binary data (\Cref{sec:data}).

\section{Discussion and Future Work}

Despite the interesting results reported in \Cref{ch:scalable}, there is much room for improvement.
We end this dissertation by addressing the flaws of both \textproc{SamplePSDD} and
