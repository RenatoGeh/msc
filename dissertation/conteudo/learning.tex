...
Notably, \citet{gens13} purposely does not strictly specify which techniques should be used for
...
independent from each other makes it an attractive candidate for CPU parallelization.
...
tuning is infeasible. \citet{rooshenas14} recommend random search \citep{bergstra12a} as an
alternative.
...
as \textproc{ID-SPN} at the input nodes, the analysis for those also hold.
...
as the choice of how to cluster and what input nodes to use is open to the user.
...
\begin{remark}[breakable]{On variations of divide-and-conquer learning}{divconq}
  ...
  and independence testing. \citet{vergari15} suggest simplifying clustering to only binary row
  splits, while \citet{liu19} propose clustering methods that automatically decide the number of
  ...
  \citet{vergari15} propose the use of Chow-Liu Trees as input nodes instead of univariate
  distributions, while \citet{molina17} recommend Poisson distributions for modeling negative
  dependence. \citet{bueff18} combine \textproc{LearnSPN} with weighted model integration by
  learning polynomials as input nodes for continuous variables and counts for discrete data.
  \citet{molina18} adapt \textproc{LearnSPN} to hybrid domains by employing the randomized
...
As the name suggests, \textproc{LearnPSDD} \citep{liang17} learns a smooth, structured decomposable
...
$\set{Y}=\Sc(v^\to)$ determines that $\set{X}$ and $\set{Y}$ are context independent, i.e.\
...
variables are nodes. For each edge $\overline{XY}$, attribute its weight as $\mutualinf(X,Y)$.
...
\begin{figure}[t]
  ...
  smoothness, (structured) decomposability and determinism are evidently preserved.}
  \label{fig:splitclone}
\end{figure}

It is easy to see that, in both cases, smoothness, structured decomposability and determinism are
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    ...
    \Ensure A smooth, structured decomposable and deterministic PC learned from $\set{D}$
...
\paragraph{Pros.} The fact that \textproc{LearnPSDD} preserves smoothness, structured
...
may take several hours for only a minor (if any) improvement. \citet{liang17} suggest improving
performance by producing ensembles of \textproc{LearnPSDD}s, although this negates determinism in
the final model (as well as structured decomposability if different vtrees are used at each
...
We first address how to construct the initial circuit from data. \citeauthor{dang20} suggest doing
...
$\set{Y}=\Sc(v^\to)$ into product nodes $\Prod_1,\ldots,\Prod_k$, one for each cached value
...
\inode[fill=boxblue!50]{\newProdNode}, where $C$ is the parent of $B$ and so we have to join the
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    \Require Data $\set{D}$, whose columns are indexed by variables $\set{X}$
    \Ensure A smooth, structured decomposable and deterministic initial PC and vtree
...
of samples $w_{\Sum,\Child}$ impacts over $\set{D}$, and thus a sense of how meaningful that
edge is on the fitness of data. As we shall see briefly, this aggregated circuit flow shall then be
...
\citet{dang20} empirically show that initial circuits constructed from \textproc{StrudelInitial}
...
\begin{remark}[breakable]{On the choice of initial circuits}{initpc}
  ...
  structured decomposable and deterministic logic circuits, who usually go by the name of Sentential
  ...
  analyze the feasibility of compiling LCs (and subsequently producing a PC by parameterizing
  ...
  resulting DNF through previously cited SDD compilers. \citet{choi17} further study route
  ...
  investigate (P)SDDs in probabilistic cardinality constraint tasks, also known as subset
...
decomposable PCs as well as structured decomposable and/or deterministic circuits through simple
modifications to their method.

\subsection{\textproc{RAT-SPN}}
\label{sec:ratspn}

A key ingredient of \textproc{RAT-SPN} \citep{peharz20a} is the concept of \emph{region graphs}.
...
the region graph is randomly built, a PC is then constructed through \textproc{CompileRegionGraph},
...
construction 2-standardized, the circuit is structured decomposable if not only the scopes of
...
and structured decomposable circuits with little change to the algorithm. More importantly, because
...
has comparable size to other state-of-the-art structure learning competitors, only when a few
dozens of these single circuits are ensembled together into a mixture do they become competitive
against state-of-the-art, meaning that in their final form, they can be tens of times the size of
PCs generated from other structure learning algorithms. Moreover, because of the number of
parameters involved in sampling these PCs ($\delta$, $k$, $t$, number of components per ensemble,
and whether to produce deterministic and/or structured decomposable PCs), a grid-search over
...
some of the more important points raised throughout this chapter. We list each algorithm seen in
\Cref{ch:learning}, describing their class, time complexity of learning the probabilistic circuit
and any other auxiliary data structure, number of hyperparameters needed during learning, whether
they accept any kind of expert knowledge in the form of logical constraints, which structural
constraints are guaranteed to hold in the resulting PCs, and which data (binary $\{0,1\}$, discrete
$\mathbb{N}$ or continuous $\mathbb{R}$) are supported. We use the same notation used throughout
this section for data dimensions: $n$ is the number of examples in a dataset and $m$ is the number
of variables of dataset. Other variable names differ in their meaning depending on each learning
technique. We next describe some of the assumptions made in order to more concisely summarize the
information set in \Cref{tab:learning}.
