...
and deterministic probabilistic circuit (cf.\ \Cref{subsection:touncertainty}), meaning its
...
$p_\mathcal{C}(\set{X},\set{Y})=p_\mathcal{C}(\set{X}) p_\mathcal{C}(\set{Y})$ for a PC $\mathcal{C}$. This
means that a PC's vtree is pivotal in embedding the independencies of the circuit's distribution.
With this in mind, \citet{liang17} propose two approaches to inducing vtrees from data, both of
which use mutual information for deciding independence. To avoid computing an exponential number of
MI terms, an approximation based on the average pairwise MI is computed instead
\begin{equation*}
  \normalfont{pMI}(\set{X},\set{Y})=
\end{equation*}
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    ...
    \foreach \i in {1,...,3} {
      \foreach \j in {2,...,3} {
        \draw[boxgray,thick] (p.corner \i) -- (p.corner \j);
      }
    }
    \foreach \i in {4,...,\ngon} {
      \foreach \j in {5,...,\ngon} {
    ...
    \draw[boxgray,thick] (p.corner 4) -- (p.corner 5);
    \draw[boxgray,thick] (p.corner 6) -- (p.corner 7);
    \draw[boxgray,thick] (p.corner 6) -- (p.corner 8);
    \draw[boxgray,thick] (p.corner 7) -- (p.corner 8);
    \foreach \i in {4,...,5} {
      \foreach \j in {6,...,\ngon} {
        \draw[thick] (p.corner \i) -- (p.corner \j);
      }
    ...
    \draw[boxgray,thick] (p.corner 2) -- (p.corner 1);
  ...
  \caption{Snapshots of four iterations from running the vtree top-down learning strategy with
    pairwise mutual information. Each iteration shows a variable partitioning, the edges that
    minimize the average pairwise mutual information in black, and the subsequent (partial) vtree.
    The algorithm finishes when all partitions are singletons.}
  \label{fig:topdownvtree}
\end{figure}

\paragraph{Top-down vtree learning.} Let $\mathcal{G}$ a fully connected graph where variables are
nodes. Each edge 
