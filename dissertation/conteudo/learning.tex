...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    ...
    \State Sample a subset $\set{Y}$ of size $t$ from $\set{X}$
    \While{$|\set{A}|<k$}
...
Determinism is not the only constraint that can be 
...
candidates. Assuming that we let the algorithm run a fixed number of iterations $i$, we get
a total runtime of $\bigo\left(i\cdot\left(t+k\cdot|\set{D}|\right)\right)$.

\begin{algorithm}[t]
  \caption{\textproc{XPC}}\label{alg:xpcs}
  \begin{algorithmic}[1]
    \Require Dataset $\set{D}$, variables $\set{X}$, min. \# of assignments per partition, \# of
      conjunctions of literals $k$, length of conjunctions $t$
