...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    ...
    % Markov 2
    \node[fill=boxteal] (a) at ($(r) + (0.75,-1)$) {$A$};
    ...
    % Markov 3
    \node[fill=boxpink!50] (f) at ($(r) + (-0.75,-1)$) {$F$};
    \node[fill=boxteal] (d) at ($(f) + (-1,0)$) {$D$};
    \node[fill=boxorange!80] (e) at ($(d) + (1,-1)$) {$E$};
    \draw (d) -- (f); \draw (e) -- (d);
    ...
    % Markov 3
    \node[fill=boxpink!50] (f) at ($(r) + (-2,-1)$) {$F$};
    \node[fill=boxteal] (d) at ($(f) + (-1,0)$) {$D$};
    \node[fill=boxorange!80] (e) at ($(d) + (1,-1)$) {$E$};
    \draw (d) -- (f); \draw (e) -- (d);
    \draw[edge] (r) -- ($(d) + (0.5,0.5)$);
...
heavy weight during learning. This, coupled with the fact that hypertuning in the huge parameter
space of \textproc{ID-SPN} must be done by a random search method, can take a heavy price in terms
of learning time.
