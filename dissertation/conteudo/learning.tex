...
\begin{remark}[breakable]{On the choice of initial circuits}{initpc}
  We only briefly mentioned in \Cref{subsection:learnpsdd} how we might want to start out with an
  ...
  subject which we briefly touched in \Cref{subsection:fromcertainty}. For this reason, smooth,
  ...
  \citep{bryant86} are natural initial circuit candidates (see \Cref{subsection:samplepsdd}).
...
then address \textproc{XPCs} \citep{dimauro21}, a flexible algorithm capable of learning smooth and
...
\textproc{CreateLayer} in \Cref{alg:ratspn} does exactly that, making sure each partition
decomposes into two distinct variable splits down to leaf region nodes. A PC is then constructed by
following \textproc{CompileRegionGraph} and passing the random region graph and number of nodes per
region and partition nodes as parameters. The function ultimately produces a dense probabilistic
circuit from the region graph blueprint, as \Cref{fig:ratspn} exemplifies.
...
Although the procedure described in \Cref{alg:ratspn} is $\bigo\left(r\cdot d(s+l)\right)$ and
therefore extremely fast, it does not paint the whole picture. The main bulk of the complexity when
learning \textproc{RAT-SPN} comes from parameter learning. \citet{peharz20a} calculate the number
of sum weights to be
\begin{equation*}
  ...
  \end{cases}
\end{equation*}
meaning that learning only the non-input parameters of \textproc{RAT-SPN} takes time $\bigo\left(
\left(r\cdot 2^d \cdot\left(s^3+s\cdot l^2\right)+r\cdot k\cdot s^2\right)\cdot|\set{D}|\right)$.
However, given that all other structure learning algorithms covered in this dissertation also
require learning circuit parameters, one might argue that the true cost of structure learning in
\textproc{RAT-SPN} is indeed subquadratic on the parameters.
