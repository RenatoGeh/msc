...
\begin{figure}[t]
  ...
  \begin{subfigure}[t]{0.45\textwidth}
    ...
    \begin{tikzpicture}
      ...
      \newSumNode[label=below:{$\{A,B,C\}$},fill=boxgreen]{s1}{$(r) + (-3.5,-3.0)$};
      \newSumNode[label=below:{$\{D,E\}$},fill=boxgreen]{s2}{$(r) + (-2.5,-2.5)$};
      \newSumNode[label=below:{$\{B,C\}$},fill=boxgreen]{s3}{$(r) + (-1.5,-2.0)$};
      \newSumNode[label=below:{$\{A\}$},fill=boxgreen]{s4}{$(r) + (-0.5,-2.0)$};
      \newSumNode[label=below:{$\{D\}$},fill=boxgreen]{s5}{$(r) + (0.5,-2.0)$};
      \newSumNode[label=below:{$\{E\}$},fill=boxgreen]{s6}{$(r) + (1.5,-2.0)$};
      \newSumNode[label=below:{$\{B\}$},fill=boxgreen]{s7}{$(r) + (2.5,-2.0)$};
      \newSumNode[label=below:{$\{C\}$},fill=boxgreen]{s8}{$(r) + (3.5,-2.0)$};
      ...
      \For{each $\Prod\in\Ch(\Sum)$}
        \For{each scope $\set{S}$ associated with $\Prod$}
          \If{$\set{S}\not\in\mathcal{H}$}
            ...
            \State $\mathcal{H}\left[\set{S}\right]\gets\Node$
          \Else
            \State Add $\mathcal{H}\left[\set{S}\right]$ as a child of $\Prod$
          \EndIf
        \EndFor
      \EndFor
...
\paragraph{Cons.} The main drawbacks of \textproc{Prometheus} 
