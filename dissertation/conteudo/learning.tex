...
expressiveness. While it is true that the application of \textproc{Split} to any product node or
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    \Require Data $\set{D}$, vtree $\vtree$, initial PC $\mathcal{C}$, max depth $m$ and max number
      of candidates $k$
    ...
    \While{there is score improvement or has not reached the iteration/time limit}
      \State Let $\mathcal{C}^\ast$ the best circuit so far, initially empty
      \State $s\gets -\infty$
      \For{each of the $k$ candidates}
        \State Let $\Sum$ the candidate for \textproc{Split} and $\Child$ the candidate for \textproc{Clone}
        \State $\mathcal{C}_i\gets\textproc{Split}(\mathcal{C},\Sum,\vtree,m)$
        \State $\mathcal{C}'\gets\textproc{Clone}(\mathcal{C}',\Child,\vtree,m)$
        \State $s'\gets\score(\set{D},\mathcal{C},\mathcal{C}')$
        \IIf{$s'>s$}{$s\gets s'$ and $\mathcal{C}^\ast\gets\mathcal{C}'$}
      \EndFor
      \State $\mathcal{C}\gets\mathcal{C}^\ast$
    \EndWhile
    \State \textbf{return} $\mathcal{C}$
  \end{algorithmic}
\end{algorithm}
...
change for the nodes affected in the transformation, allowing \textproc{LearnPSDD} to cache values.
...
\textproc{Clone}, any PC is eligible as an initial circuit, even the ones which do not respect any
...
may take several hours for only minor (if any) improvement. \citep{liang17} suggests improving
performance by producing ensembles of \textproc{LearnPSDD}s, although this negates determinism in
the final circuit. Another issue is with the choice of an initial circuit. As previously mentioned,
any circuit will do, however the performance (and efficiency) of \textproc{LearnPSDD} is highly
dependent on the choice of initial circuit. In the context of PSDDs and encoding their support,
\textproc{LearnPSDD} requires that a separate algorithm compiles an LC for a specific task.
Although there are many ways of doing so, they are often not task agnostic (see \Cref{rem:initpc}).
More importantly, because the process of learning the circuit is decoupled from the task of
encoding logical constraints imposed by a knowledge base, all variables that do not appear in
the logic formula are compiled into a trivial form (e.g.\ fully factorized circuit).
