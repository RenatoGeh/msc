...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    ...
    \While{there is score improvement or has not reached the iteration/time limit}
      ...
      \State Let $\Sum^\ast$ the best \textproc{Split} candidate seen so far, initially empty
      \For{each candidate $\Sum$ of all possible \textproc{Split} candidates}
        \State $\mathcal{C}'\gets\textproc{Split}(\mathcal{C},\Sum,\vtree,m)$
        \State $s'\gets\score(\set{D},\mathcal{C},\mathcal{C}')$
        \IIf{$s'>s_{\Sum}$}{$s_{\Sum}\gets s'$ and $\Sum^\ast\gets\Sum$}
      \EndFor
      ...
      \EndFor
      \IIf{$s_{\Sum}>s_{\Child}$}{$\mathcal{C}\gets\textproc{Split}(\mathcal{C},\Sum^\ast,\vtree,m)$}
...
mainly improves \textproc{LearnPSDD} on three fronts: (1) by providing a simple algorithm for
