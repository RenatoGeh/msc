...
We first look at \textproc{RAT-SPN} \citep{peharz19}, a connectionist PC structure learning
...
A key ingredient to \textproc{RAT-SPN} \citep{peharz19} is the concept of \emph{region graphs}.
...
\Cref{alg:ratspn}). In fact, \citep{peharz19} argue that the parameterization of the circuit by
means of sum weights are as important as its structure. Indeed, they show that this connectionist
approach heavily inspired by neural networks produces very competitive PCs. However, to do so
requires extensive optimization of the circuit's weights, which unsurprisingly is where
\textproc{RAT-SPN} shines: because of the tensorized nature of region graphs, the resulting PC is
able to exploit the advantages of known deep learning frameworks, making the most of efficient
stochastic gradient descent optimizers and GPU parallelization.
...
children) and decomposable (every partition splits variables into two regions).
